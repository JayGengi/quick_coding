...
import com.guoxun.airbaba.ui.adapter.mine.CollectionMerchantsAdapter
...
/**
  ...
  */
class CollectionMerchantsFragment : BaseFragment(){

    private var type: Int? = 1
    private var baseList = ArrayList<String>()
    private val mAdapter by lazy { activity?.let { CollectionMerchantsAdapter( baseList) } }
//    private val mPresenter by lazy { FollowListPresenter() }

    companion object {
        fun setInstance(type: Int): CollectionMerchantsFragment {
            val fragment = CollectionMerchantsFragment()
            val bundle = Bundle()
