package com.guoxun.demo.widget.calendar;
...
class SignCalendar extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 1; i <= selectMonthMaxDay; i++) {
            ...
            if (signRecords != null) {
                ...
                for(int da = 0; da<signRecords.size(); da ++){
                     if(dateStr.equals(signRecords.get(da).getData()) && 1 == signRecords.get(da).getIs_sign()){
                        paint.setColor(config.signColor);
                        canvas.drawCircle(x, y, config.signSize / 2, paint);
                        isSign = true;
                    }else if(dateStr.equals(signRecords.get(da).getData()) && 2 == signRecords.get(da).getIs_sign()){
                        paint.setColor(config.errorColor);
                        canvas.drawCircle(x, y, config.signSize / 2, paint);
                        isSign = false;
                    }
                }
...
//                }
            ...
            if (isSign) {
                for(int da = 0; da<signRecords.size(); da ++){
                    if( 1 == signRecords.get(da).getIs_sign()){
...
//                paint.setColor(config.signTextColor);
                ...
                for(int da = 0; da<signRecords.size(); da ++){
                     if( 2 == signRecords.get(da).getIs_sign()){
                        paint.setColor(config.errorTextColor);
                    }
                }
            }
            paint.setTextSize(config.calendarTextSize);
    ...
    void setSignRecords( List<CalendarEntity> signRecords) {
